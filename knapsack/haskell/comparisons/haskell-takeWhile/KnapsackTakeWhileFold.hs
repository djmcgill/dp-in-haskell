...
module KnapsackTakeWhileFold where
...
knapsackScaled vws (W cap) = V.unsafeIndex solns cap
    ...
    solns = V.generate (cap+1) getBest
        where
        -- i is the current index, less is the maximal index into vws such that forall ix. 0 <= ix < less', getW (vws[ix]) <= W i
        getBest !i = maxSolution
            where
            maxSolution = foldr1 eachPair emptySoln (U.toList vws) of
                Left s -> s
                Right _ -> emptySoln


            --eachPair :: Either Solution -> (Int, (Value, Weight)) -> Either Solution
            -- TODO: check if we've reached the end or need to check the next
            eachPair (ix, (v, w)) s
                | i >= unW w = case solns `V.unsafeIndex` (i - unW w) of
                    Solution s' v' w' -> max s $ Solution (ix : s') (v' + v) (w' + w)
                | otherwise = s
