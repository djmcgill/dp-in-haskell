...
knapsackScaled vws (W cap) = V.unsafeIndex solns cap
    ...
    solns = snd $ T.mapAccumL getBest 0 (V.enumFromN (0 :: Weight) (cap+1))
        where
        -- i is the current index, validIx is the maximal index into vws such that
        -- forall ix. 0 <= ix < validIx', getW (vws[ix]) <= W i
        getBest :: Int -> Weight -> (Int, Solution)
        getBest !validIx !(W i) = (validIx', maxSolution)
            where
            ...
            eachPair (ix, (v, w)) = case solns `V.unsafeIndex` (i - unW w) of -- XXX: this is getting oob error
