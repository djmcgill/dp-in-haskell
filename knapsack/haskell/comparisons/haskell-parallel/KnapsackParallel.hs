...
knapsackScaled vws (W cap) = V.unsafeIndex solns cap
    ...
    solns = flip evalState 0 $ V.generateM (cap+1) $ \i -> do
        ...
        let eachPair :: (Int, (Value, Weight)) -> Solution
            ...
            bestSolution = runPar $ parMapReduceRangeThresh 500 range oneSolution combineSolutions emptySoln
                where
                oneSolution i = return $ eachPair (validWeights U.! i)
                combineSolutions s1 s2 = return (max s1 s2)
                range = InclusiveRange 0 (U.length validWeights - 1)
        return bestSolution
